...
_Boot_Test:
	...
	ar1 = 0x01
	ar0 = 0x00
;	./src/main.c:7: printf("welcome to use\r\n");
	mov	a,#___str_0
	push	acc
	mov	a,#(___str_0 >> 8)
	push	acc
	...
	dec	sp
	dec	sp
;	./src/main.c:8: if(Wdt_Runsign())printf("boot: Start self test[pass]\r\n");
	lcall	_Wdt_Runsign
	mov	a,dpl
	jz	00102$
	mov	a,#___str_1
	...
	sjmp	00103$
00102$:
;	./src/main.c:11: printf("boot: Start self test[WDT Operation sign NG ]\r\n");
	mov	a,#___str_2
	push	acc
	mov	a,#(___str_2 >> 8)
	push	acc
	...
	dec	sp
	dec	sp
;	./src/main.c:12: Lcd1602_Clean();
	lcall	_Lcd1602_Clean
;	./src/main.c:13: Lcd1602_Print(0,1,"dht11 error ->");
	mov	_Lcd1602_Print_PARM_3,#___str_3
	mov	(_Lcd1602_Print_PARM_3 + 1),#(___str_3 >> 8)
	mov	(_Lcd1602_Print_PARM_3 + 2),#0x80
	mov	_Lcd1602_Print_PARM_2,#0x01
	mov	dpl,#0x00
	lcall	_Lcd1602_Print
;	./src/main.c:14: delayms(2000);
	mov	dptr,#0x07d0
	lcall	_delayms
00103$:
...
;------------------------------------------------------------
;	./src/main.c:30: void show_data()
;	-----------------------------------------
;	 function show_data
;	-----------------------------------------
_show_data:
;	./src/main.c:32: Lcd1602_Print(0,0,"Tem:");
	...
	mov	dpl,#0x00
	lcall	_Lcd1602_Print
;	./src/main.c:33: Lcd1602_Print(7,0,".");
...
;	./src/main.c:37: Lcd1602_PrintCust(9,0,temperature);
	...
	mov	dpl,#0x09
	lcall	_Lcd1602_PrintCust
;	./src/main.c:39: Lcd1602_PrintNumber(6,0,the);
...
;	./src/main.c:40: Lcd1602_PrintNumber(8,0,thef);
	mov	_Lcd1602_PrintNumber_PARM_2,#0x00
	mov	_Lcd1602_PrintNumber_PARM_3,_thef
	mov	(_Lcd1602_PrintNumber_PARM_3 + 1),(_thef + 1)
	mov	dpl,#0x08
	lcall	_Lcd1602_PrintNumber
;	./src/main.c:41: Lcd1602_PrintNumber(8,1,hum);
	mov	_Lcd1602_PrintNumber_PARM_2,#0x01
	mov	_Lcd1602_PrintNumber_PARM_3,_hum
	mov	(_Lcd1602_PrintNumber_PARM_3 + 1),(_hum + 1)
	mov	dpl,#0x08
	lcall	_Lcd1602_PrintNumber
;	./src/main.c:43: if(subzero)
	mov	a,_subzero
	orl	a,(_subzero + 1)
	jz	00103$
;	./src/main.c:44: Lcd1602_Print(4,0,"-");
	mov	_Lcd1602_Print_PARM_3,#___str_11
	mov	(_Lcd1602_Print_PARM_3 + 1),#(___str_11 >> 8)
	mov	(_Lcd1602_Print_PARM_3 + 2),#0x80
	mov	_Lcd1602_Print_PARM_2,#0x00
	mov	dpl,#0x04
;	./src/main.c:45: }
...
;------------------------------------------------------------
;	./src/main.c:50: void debug()
;	-----------------------------------------
;	 function debug
;	-----------------------------------------
_debug:
;	./src/main.c:52: printf("read back time %d\r\n",time);
	push	_time
	push	(_time + 1)
	mov	a,#___str_12
	push	acc
...
;	./src/main.c:56: printf("check: %d\r\n",check);
	...
	add	a,#0xfb
	mov	sp,a
;	./src/main.c:57: printf("subzero: %d\r\n",subzero);
	push	_subzero
	push	(_subzero + 1)
	mov	a,#___str_17
	push	acc
...
;------------------------------------------------------------
;	./src/main.c:59: void main ()
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
;	./src/main.c:61: Wdt_Init();
	lcall	_Wdt_Init
;	./src/main.c:62: Int_Init();     	//初始化系统中断
	lcall	_Int_Init
;	./src/main.c:63: Time0_Init();		//初始化定时器0
	lcall	_Time0_Init
;	./src/main.c:64: Time1_Init();		//初始化定时器1
	lcall	_Time1_Init
;	./src/main.c:65: Serial_Init();		//串口初始化
	lcall	_Serial_Init
;	./src/main.c:66: Time2_Init();		//初始化定时器2
	lcall	_Time2_Init
;	./src/main.c:67: Lcd1602_Init();		//初始化lcd1602
	lcall	_Lcd1602_Init
;	./src/main.c:68: Lcd1602_Clean();	//清空lcd1602显存
	lcall	_Lcd1602_Clean
;	./src/main.c:69: print_boot();		//显示欢迎界面(等待DHT11初始化完成)
	lcall	_print_boot
;	./src/main.c:70: Boot_Test();		//开机自检
	lcall	_Boot_Test
;	./src/main.c:71: while(1){
00102$:
;	./src/main.c:72: Wdt_Clean();
	lcall	_Wdt_Clean
;	./src/main.c:73: DHT11_Read();
	lcall	_DHT11_Read
;	./src/main.c:74: Lcd1602_Init();
	lcall	_Lcd1602_Init
;	./src/main.c:75: Lcd1602_Clean();
	lcall	_Lcd1602_Clean
;	./src/main.c:76: show_data();
	lcall	_show_data
;	./src/main.c:77: show_time();
	lcall	_show_time
;	./src/main.c:78: time++;
	inc	_time
	clr	a
...
___str_1:
	...
	.area CONST   (CODE)
___str_2:
	.ascii "boot: Start self test[WDT Operation sign NG ]"
	.db 0x0d
	.db 0x0a
	.db 0x00
	.area CSEG    (CODE)
