...
;------------------------------------------------------------
;	./src/dht11.c:46: char DHT11_Read(void)
;	-----------------------------------------
;	 function DHT11_Read
;	-----------------------------------------
_DHT11_Read:
;	./src/dht11.c:49: mixdata=0;
	clr	a
	mov	_mixdata,a
	mov	(_mixdata + 1),a
	mov	(_mixdata + 2),a
	mov	(_mixdata + 3),a
;	./src/dht11.c:50: check=0;
	mov	_check,a
	mov	(_check + 1),a
;	./src/dht11.c:52: DHT11_Start();
	lcall	_DHT11_Start
;	./src/dht11.c:53: DHT11_Data();
	lcall	_DHT11_Data
;	./src/dht11.c:55: hum=(mixdata>>24)&0xff;
	mov	r7,(_mixdata + 3)
	mov	_hum,r7
	mov	(_hum + 1),#0x00
;	./src/dht11.c:56: the=(mixdata>>8)&0xff;
	mov	r6,(_mixdata + 1)
	mov	_the,r6
	mov	(_the + 1),#0x00
;	./src/dht11.c:57: thef=(mixdata>>0)&0xf;
	mov	r6,_mixdata
	mov	a,#0x0f
	anl	a,r6
	mov	_thef,a
	mov	(_thef + 1),#0x00
;	./src/dht11.c:59: subzero=(mixdata>>7)&0x1;
	mov	a,_mixdata
	rl	a
	anl	a,#0x01
	mov	r7,a
	mov	_subzero,r7
	mov	(_subzero + 1),#0x00
;	./src/dht11.c:61: if(check!=hum+the+thef)return -1;
	mov	a,_the
	add	a,_hum
	mov	r6,a
	mov	a,(_the + 1)
	...
	cjne	a,_check,00109$
	mov	a,r7
	cjne	a,(_check + 1),00109$
	sjmp	00102$
00109$:
	mov	dpl,#0xff
	ret
00102$:
;	./src/dht11.c:62: return 0;
	mov	dpl,#0x00
;	./src/dht11.c:63: }
	ret
	.area CSEG    (CODE)
